...
function PANEL:Init()
    self:Receiver("inventorySystem", function(self, tbl, dropped, menuIndex, x, y)
        if not dropped then
        ...
        if(not IsValid(originSlot.Inventory) or not IsValid(self.Inventory)) then
            return 
        end
    ...
    hook.Add("inventorySystem.HideLocalInventory", self, function(self)
        self.TextHoverAlpha = 0
    end)
end
...
function PANEL:Think()
    if(dragndrop.IsDragging()) then
        ...
        if(self:IsHovered() and item:CanAddToInventory(self.Inventory, self.SlotId)
            and hook.Run("inventorySystem.CanTransferTo", itemInventory, itemSlot, myInventory, mySlotId) != false) then
            self.InternalColor = self.ColorHighlighted
        else
            self.InternalColor = self.ColorNormal
        end
...
function PANEL:Paint(w, h)
    ...
    if(self.HoverInfo and not self.Item) then
        ...
        local x, y = surface.GetTextSize(self.HoverInfo)
...
function PANEL:GenerateItem(inventoryToUse)
    ...
    if not IsValid(inventoryToUse) then
        return
    end
...
function PANEL:Init()
    ...
    self:Receiver("inventorySystem", function(self, tbl, dropped, menuIndex, x, y)
        ...
        local originSlot = tbl[1]:GetParent()
        local targetSlot = self:GetParent()

        if(not IsValid(originSlot.Inventory) or not IsValid(targetSlot.Inventory)) then
            return 
        end
...
function PANEL:OnCursorEntered()
    if self:IsDragging() || IsValid(self.DescBox) then return end
