...
function inventoryMeta:SendFullUpdateToPlayer(plys)
    self:SendUpdateToPlayer(self.ValidSlots, plys)
end
...
function inventoryMeta:SendUpdateToPlayer(slots, plys)
    ...
    for _, slot in ipairs(slots) do
        if(self.Contents[slot] == nil) then
            ...
            info[slot] = self.Contents[slot]:GetItemId()
        end
    end
...
net.Receive("inventorySystem.Drop", function(len, ply)
    if((ply.NextInventoryMovement or 0) > CurTime()) then
        return 
    end
    ...
    if not itemInventory.ValidSlots[itemSlot] then
        return 
    end

    if(itemInventory:GetParent() != ply and itemInventory:GetPos():DistToSqr(ply:GetPos()) > math.pow(inventorySystem.InteractionRange, 2)) then
        return
    end

    if hook.Run("inventorySystem.CanDrop", itemInventory, itemSlot) == false then
        return 
    end

    local itemToRemove = itemInventory:RemoveItem(itemSlot)
    if(not IsValid(itemToRemove)) then
        return 
    end
...
net.Receive("inventorySystem.Movement", function(len, ply)
    ...
    if not targetInventory.ValidSlots[targetSlot] then
        return 
    end

    if not IsValid(itemInventory:GetParent()) or not IsValid(targetInventory:GetParent()) then
        return 
    end

    if(itemInventory:GetPos():DistToSqr(targetInventory:GetPos()) > math.pow(inventorySystem.InteractionRange, 2)) then
        return
    end

    if hook.Run("inventorySystem.CanTransferTo", itemInventory, itemSlot, targetInventory, targetSlot) == false then
        return 
    end
    local itemToCheck = targetInventory:GetItem(targetSlot)
    if(IsValid(itemToCheck) and not itemToCheck:CanAddToInventory(itemInventory, itemSlot)) then
        return 
    end
    itemToCheck = itemInventory:GetItem(itemSlot)
    if(IsValid(itemToCheck) and not itemToCheck:CanAddToInventory(targetInventory, targetSlot)) then
        return
    end

    local removedItem = itemInventory:RemoveItem(itemSlot)
    if(not IsValid(removedItem)) then
        return 
    end
    ...
    if(IsValid(itemToSwap)) then
        targetInventory:AddItem(itemSlot, itemToSwap)
    end
end)