...
function ITEM:Initialize() // Called when the Item first loads. 
    if CLIENT then
        self.DermaDesc.ExtraHeight = self.DermaDesc.ExtraHeight or 12
        ...
        self.DermaDesc.SizeY = 58 + self.ParseObjDesc:GetHeight() + self.ParseObjTitle:GetHeight() + self.DermaDesc.ExtraHeight
...
function ITEM:__tostring()
    if(self == inventorySystem.InvalidItem or self.__Deleted) then
        return "[Deleted Item "..tostring(self:GetItemId()).."]"
    else
        return "Item ["..self:GetItemId().."]["..self:GetName().."]"
    end
end
...
function ITEM:SetNetworkedVariable(varName, value)
    ...
    if(varName == "Int" or varName == "UInt") then
...
function ITEM:UpdateDescBoxDimensions()
    self.DermaDesc.SizeY = 58 + self.ParseObjDesc:GetHeight() + self.ParseObjTitle:GetHeight() + self.DermaDesc.ExtraHeight
    if(self.InventoryLocation == LocalPlayer():GetInventory()) then
        inventorySystem.UpdatePlayerInventory(self.InventoryLocation)
    end
end
...
function ITEM:CreateNetworkedVariable(name, netType) 
    ...
    self["Get"..name] = function(self, value)
        return self[name]
    end
end
...
function ITEM:RenderModel(x1, y1, x2, y2)
    ...
    local cameraPos = Vector(size, 0, 0)
    if(not self.DrawModelSide) then
        cameraPos = Vector(0, size, 0)
    end
    local angle = (cameraTarget - cameraPos):Angle()

    cam.Start3D( cameraPos, angle, 60, x1, y1, width, height, 5, self.FarZ )
...
function ITEM:CanAddToInventory(inventory, slot)
    ...
    if(not self.SlotsAllowed[slotType]) then
        return false 
    end
    return true
end
...
function ITEM:CreatePhysical()
    if(self.InventoryLocation) then
...
function ITEM:DrawItem(panel, width, height)
    ...
    local x1, y1 = panel:LocalToScreen(6, 28)
    local x2, y2 = panel:LocalToScreen(width - 12, height)
...
function ITEM.Derma:OnMousePressed(keyCode)
    ...
    if (keyCode == MOUSE_RIGHT) then
        ...
        if(menuToOpen:ChildCount() <= 0) then
...
if(SERVER) then
    ...
    net.Receive("inventorySystem.PerformAction", function(len, ply)
        ...
        if not item:CanPerformAction(action, ply) then
            return
        end
