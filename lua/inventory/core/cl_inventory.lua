...
local function Empty(self, width, height) end
...
function GenerateHoldingFrame()
    function DropItem(pnl, tbl, dropped, menuIndex, x, y)
        ...
        if(not IsValid(parent.Inventory)) then
            return 
        end
        ...
        function AnimatePlayer(self)
            if(LocalPlayer():Alive()) then
                ...
                for _, parameter in ipairs(poseParameters) do
                    ...
                    self:SetPoseParameter(parameter, newValue)
                end
            else
                for _, parameter in ipairs(poseParameters) do
                    self:SetPoseParameter(parameter, 0)
                end
            end
        end
...
function GeneratePlayerPanel()
    ...
    function playerPanel.dPlayer:PostDrawModel(ent)
        ...
        if(not IsValid(activeWeapon) or activeWeapon:GetWeaponWorldModel() == "" or not activeWeapon:GetWeaponWorldModel()) then 
            return 
        end
...
function GenerateSlots()
    ...
    inventorySystem.slotLength = math.min(
        math.min(
            inventorySystem.holdingFrame:GetWide() / (2560 * screenScale),
            inventorySystem.holdingFrame:GetTall() / (1440 * screenScale)
        ) * inventorySystem.maxSlotLength, 
        (storageSlots:GetWide() - 10) / maxXSlots - inventorySystem.slotSpacing
    )
    ...
    local allSlots = {}
    local MakeSlot = inventorySystem.MakeSlotFunction(allSlots, inventorySystem.localPlayerInventory, 2)
    ...
    inventorySystem.storageFrame:SetPos(
        inventorySystem.holdingFrame:GetWide() + inventorySystem.holdingFrame:GetX(), 
        inventorySystem.holdingFrame:GetY() + (inventorySystem.slotSpacing + inventorySystem.slotLength) * maxStorageRows
    )
    inventorySystem.storageFrame:InvalidateLayout(true)
    inventorySystem.storageFrame:SetSize(
        prevX - inventorySystem.holdingFrame:GetWide(), 
        (inventorySystem.slotSpacing + inventorySystem.slotLength) * maxStorageRows - inventorySystem.slotSpacing
    )

    function inventorySystem.UpdatePlayerInventory(inventory)
        for slot, panel in pairs(allSlots) do
            panel:GenerateItem(inventory)
        end
    end
end
...
local function GenerateInventoryUI(self)
...
hook.Add("OnGamemodeLoaded", inventorySystem, GenerateInventoryUI)
...
// For autorefresh support
if inventorySystem.InitializedUI == true then
    GenerateInventoryUI(inventorySystem)
end