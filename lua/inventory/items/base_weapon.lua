...
function ITEM:Initialize(weapon)
    ...
    if(SERVER) then
        ...
        hook.Add("PlayerCanPickupWeapon", self, function(self, ply, weapon)
            ...
            if inventory:GetParent() == ply and inventorySystem.GetSlotType(self.SlotLocation) == "weapon" then
                return true
            end
            return false
        end)
        ...
        self.Weapon:CallOnRemove("weapon_inventorysystem_item", function(weapon)
...
function ITEM:CanAddToInventory(inventory, slot)
    ...
    if(not self:GetWeaponClass()) then
        return false
    end

    if(inventorySystem.GetSlotType(slot) == "weapon") then
        if(not IsValid(inventory:GetParent()) or inventory:GetParent():HasWeapon(self:GetWeaponClass())) then
            return false
        end
    end
    return true
...
function ITEM:GiveWeapon()
    ...
    local inventory = self.InventoryLocation
    if not IsValid(inventory) or not IsValid(inventory:GetParent()) then
        return
    end
